...
//#define newPress(int channel, bool last) (last ? false : vexRT[channel]);
...
void lockBase(int position) {
	setDrive(-SensorValue[rightQuad] + position);
}
...
task main()
{
	...
	while (true) {
		...
		//	lockedBase = vexRT[Btn7U] ? false : true;
		//	currentLock = SensorValue[rightQuad];
		//} else {
		//	lockBase(currentLock);
		//	lockedBase = vexRT[Btn7U] ? true : false;
		//}

		dir = newPress(Btn8U, last) ? !dir : dir;
		...
		setIntake(buttonToPower(vexRT[Btn5D],vexRT[Btn5U], 127));
